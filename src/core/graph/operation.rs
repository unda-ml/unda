...
impl Hash for Operation {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        match self {
            ...
            Self::Select { pred, on_true, on_false } => {
                pred.hash(state);
                on_true.hash(state);
                on_false.hash(state);
            },
            ...
            Self::SliceInDim { node, start, stop, stride, dim } => {
                node.hash(state);
                start.hash(state);
                stop.hash(state);
                stride.hash(state);
...
impl PartialEq for Operation {
    fn eq(&self, other: &Self) -> bool {
        return match (&self, &other) {
            ...
            (&Self::Select { pred, on_true, on_false }, 
             &Self::Select { pred: pred2, on_true: on_true2, on_false: on_false2 }) => {
                pred == pred2 && on_true == on_true2 && on_false == on_false2
            }
            (&Self::TypeCast(a, ty), &Self::TypeCast(b, ty2)) => {
                a == b && ty == ty2
            }
            (&Self::Transpose(a, dim), &Self::Transpose(b, dim2)) => {
                a == b && dim == dim2
            }
            (&Self::SliceInDim { node, start, stop, stride, dim },
             &Self::SliceInDim { node: node2, start: start2, stop: stop2, stride: stride2, dim: dim2 }) => {
                node == node2 &&
                    start == start2 &&
                    stop == stop2 &&
                    stride == stride2 &&
            ...
            (&Self::TileInDim { node, n_tiles, dim },
             &Self::TileInDim { node: node2, n_tiles: n_tiles2, dim: dim2 }) => {
                node == node2 && 
                    n_tiles == n_tiles2 && 
                    dim == dim2
            }
            (&Self::ReduceMax { node, dim }, &Self::ReduceMax { node: node2, dim: dim2 })
                | (&Self::ReduceMean { node, dim }, &Self::ReduceMean { node: node2, dim: dim2 })
                | (&Self::ReduceSum { node, dim }, &Self::ReduceSum { node: node2, dim: dim2 }) => {
                    node == node2 &&
                        dim == dim2
                }
            _ => false
