...
impl Matrix{
    ...
    fn par_multiply(&self, other: &Matrix) -> Matrix {
        ...
        let p1 = rayon::join(|| a11.multiply(&(b12.clone() - &b22)), || a11.multiply(&(b12.clone() - &b22)));
        let p2 = rayon::join(|| (a11.clone() + &a12).multiply(&b22), || (a11.clone() + &a12).multiply(&b22));
        let p3 = rayon::join(|| (a21.clone() + &a22).multiply(&b11), || (a21.clone() + &a22).multiply(&b11));
        let p4 = rayon::join(|| a22.multiply(&(b21.clone() - &b11)), || a22.multiply(&(b21.clone() - &b11)));
        let p5 = rayon::join(
            || (a11.clone() + &a22).multiply(&(b11.clone() + &b22)),
            || (a11.clone() + &a22).multiply(&(b11.clone() + &b22)),
        );
        let p6 = rayon::join(
            || (a12.clone() - &a22).multiply(&(b21.clone() + &b22)),
            || (a12.clone() - &a22).multiply(&(b21.clone() + &b22)),
        );
        let p7 = rayon::join(
            || (a11.clone() - &a21).multiply(&(b11.clone() + &b12)),
            || (a11.clone() - &a21).multiply(&(b11.clone() + &b12)),
        );

        // Calculate result matrices
        let c11 = rayon::join(
            || p5.0.clone() + &p4.0 - &p2.0 + &p6.0,
            || p5.1.clone() + &p4.1 - &p2.1 + &p6.1,
        );
        let c12 = rayon::join(|| p1.0.clone() + &p2.0, || p1.1.clone() + &p2.1);
        let c21 = rayon::join(|| p3.0.clone() + &p4.0, || p3.1.clone() + &p4.1);
        let c22 = rayon::join(
            || p5.0.clone() + &p1.0 - &p3.0 - &p7.0,
    ...
    fn multiply(&self, other: &Matrix) -> Matrix {
        ...
        let p1 = a11.multiply(&(b12.clone() - &b22));
        let p2 = (a11.clone() + &a12).multiply(&b22);
        let p3 = (a21.clone() + &a22).multiply(&b11);
        let p4 = a22.multiply(&(b21.clone() - &b11));
        let p5 = (a11.clone() + &a22).multiply(&(b11.clone() + &b22));
        let p6 = (a12.clone() - &a22).multiply(&(b21.clone() + &b22));
        let p7 = (a11.clone() - &a21).multiply(&(b11.clone() + &b12));

        // Calculate result matrices
        let c11 = p5.clone() + &p4 - &p2 + &p6;
        let c12 = p1.clone() + &p2;
        let c21 = p3.clone() + &p4;
        let c22 = p5.clone() + &p1 - &p3 - &p7;
